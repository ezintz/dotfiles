...
The run order in `bin/dotfiles` is: `migrate_overlay` → sync (this repo, then the overlay if it is a checkout) → tracked packages (1Password first, then waits for its sign-in and the App Store's — `prepare_sign_ins`) → `setup_overlay` → the overlay's `Brewfile` → links and Claude → `setup_git` → `bin/_macos` → `backup_overlay`. Each position is load-bearing: the App Store and the forge login need passwords from 1Password, which the Brewfile itself installs; `setup_overlay` needs `gh`/`glab`/`tea` from the tracked Brewfile, and has to restore the overlay before its Brewfile, settings and `gitauthor` are read; `backup_overlay` runs last so it sees the `gitauthor` that `setup_git` just wrote.
...
Both terminals carry the same OneDark palette — `ghostty/config` and, in `iterm2/com.googlecode.iterm2.plist`, **two** copies of every colour key: the `Default` profile and the `OneDark` custom preset. They must move together. There used to be a third, a dedicated `tmux` profile that `TmuxUsesDedicatedProfile` sent tmux-integration sessions to; it is gone, and `bin/_macos` now sets that key explicitly to `false` rather than dropping the line, so a machine still carrying `true` stops pointing at a profile that no longer exists. `iterm2/OneDark.itermcolors` is a third copy, importable but never read at runtime. Two slots deliberately differ from what was originally imported; `ghostty/config` carries the reasoning at the palette block. Edit the plist only while iTerm2 is **not running**, or it writes its in-memory preferences back over the file on quit and the change vanishes with no error. Its components are float32 widened to double (`118/255` is stored as `0.46274510025978088`), so a value computed in double precision will not match and a search-and-replace will silently find nothing; `Only The Default BG Color Uses Transparency` is `true`, so iTerm2 applies transparency to the window background alone and keeps panel fills painted by a TUI opaque. Ghostty deliberately differs: `background-opacity-cells = true` makes those fills translucent too, because Claude Code's changes pane is a fill the theme cannot remove, only recolour to match the window.
