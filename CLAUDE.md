...
- `ghostty/` holds the config cmux renders with. cmux is a native app embedding libghostty and deliberately surfaces no font or cursor settings of its own, so everything about how a cmux pane looks is in `ghostty/config`; cmux's own `~/.config/cmux/cmux.json` covers only app behaviour. Unlike the iTerm2 plist this one *is* symlinked (per file, from `bin/dotfiles`), so cmux's "Open Ghostty Settings" command edits the repo copy directly.
