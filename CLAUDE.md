...
- `themes/onedark.json` → `~/.claude/themes/onedark.json`, selected by `"theme": "custom:onedark"` in `settings.json`. Its `base` is `dark`, not `dark-ansi`, and that choice is load-bearing: the diff renderer picks its colour mode from the base *name*, and a name containing "ansi" forces every value — including a hex override — through a cube-vs-greyscale nearest-neighbour quantiser. For any dark desaturated tint the greyscale ramp wins, so all four diff fills collapsed to the same neutral `#303030` and added was indistinguishable from removed. Across all 240 entries of the 256-colour palette exactly one works as a red fill and **none** as a green one — measured as surviving the quantiser unchanged, holding text at 4.5:1, and staying visible against the panel — so a readable green diff row does not exist on an ansi base at any value. That is what rules the ansi bases out; everything else here renders truecolor on either base. The cost of `dark` is that unlisted tokens come from Claude's built-in theme rather than the terminal palette, which is why the semantic hues (`success`, `error`, `warning`, `permission`, `planMode`, `autoAccept`, the eight `*_FOR_SUBAGENTS_ONLY`) are all pinned to OneDark values here; only the decorative `rainbow_*` tokens are left inherited, since they are meant to be a rainbow. Two traps when editing it. An override naming a token the base lacks is **silently dropped**, so verify a new key against the base rather than trusting a clean start. `diffAddedWord` and `diffRemovedWord` are **foreground** colours despite sitting among the fills — they also colour the `+N`/`-N` counts in the sidebar header — so they need contrast *against* `composerSidebarBackground`, not text placed on top of them; tuned as fills they came out at 1.7:1 and were unreadable. The four real fills must in turn be checked against two backgrounds — the pane (`#21252B`) and `composerSidebarBackground` (`#282C34`) — because the sidebar is the lighter of the two and a fill tuned only against the pane disappears there. Claude Code's `/theme` editor writes a theme back by renaming a temp file over it, which replaces the symlink with a regular file — a theme tweaked in the UI detaches from this repo until `bin/dotfiles` re-links it.
